...
function getUser(nPage) {
    CS('CZCLZ.FHRSHMag.GetFHRList', function (retVal) {
        if (retVal){
                ...
                buttons: [
                    {
                        ...
                        id:"btn1",
                        handler: function () {
                            if (privilege("专线审核_发货人审核_编辑")) {
                                Ext.MessageBox.confirm('提示', '是否通过!', function (obj) {
                                    if (obj == "yes") {
                                        CS('CZCLZ.FHRSHMag.SHFHRCG', function (retVal) {
                                            if (retVal) {
                                                var result = retVal.evalJSON();
                                                getUser(1);
                                                if (result.success) {
                ...
                buttons: [
                    {
                        ...
                        handler: function () {
                            if (privilege("专线审核_发货人审核_编辑")) {
                                CS('CZCLZ.FHRSHMag.SHFHRSB', function (retVal) {
                                    if (retVal) {
                                        var result = retVal.evalJSON();
                                        getUser(1);
                                        if (result.success) {
                                             ...
                                             items: [
                                                 {
                                                     ...
                                                     text: '导出',
                                                     handler: function () {
                                                         if (privilege("专线审核_发货人审核_导出")) {
                                                             DownloadFile("CZCLZ.FHRSHMag.GetFHRList2ToFile", "发货人审核.xls", Ext.getCmp("cx_yhm").getValue(), Ext.getCmp("cx_xm").getValue(), Ext.getCmp("cx_ispass").getValue());
                                                         }
                                                     }
                                                 }]
                                         }
