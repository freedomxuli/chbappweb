...
var pageSize = 15;
//-------------------------------------------------------数据源------------------------------------------------------------
var storeDriverPay = createSFW4Store({
    data: [],
    pageSize: pageSize,
    total: 1,
    currentPage: 1,
...
//-------------------------------------------------------页面方法----------------------------------------------------------
//获取订单列表
function GetDriverPay(nPage) {
    purPage = nPage;

    CS('CZCLZ.Order.GetDriverPayByPage', function (retVal) {
        storeDriverPay.setData({
            data: retVal.dt,
            pageSize: pageSize,
            total: retVal.ac,
                    ...
                    buttons: [
                        {
                            ...
                            handler: function () {
                                ...
                                if (form.form.isValid()) {
                                    ...
                                    CS('CZCLZ.Order.SaveOrderPay', function (retVal) {
                                        if (retVal) {
                                            GetDriverPay(purPage);
...
function SubmitDriverPay(id) {
    if (privilege("一键发包模块_订单管理-司机付款列表_提交财务申请付款")) {
        Ext.MessageBox.confirm('添加提示', '是否要提交财务申请付款!', function (obj) {
            if (obj == "yes") {
                CS('CZCLZ.Order.UpdateOrderPaySubmit', function (retVal) {
                    if (retVal) {
                        GetDriverPay(purPage);
                    }
                }, CS.onError, [id]);
...
function ConfirmDriverPay(id) {
    if (privilege("一键发包模块_订单管理-司机付款列表_财务确认付款")) {
        Ext.MessageBox.confirm('添加提示', '是否要财务确认付款!', function (obj) {
            if (obj == "yes") {
                CS('CZCLZ.Order.UpdateOrderPayConfirm', function (retVal) {
                    if (retVal) {
                        GetDriverPay(purPage);
                    }
                }, CS.onError, [id]);
...
function DeleteDriverPay(id, orderid) {
    if (privilege("一键发包模块_订单管理-承运商付款申请_删除")) {
        Ext.MessageBox.confirm('删除提示', '是否要删除数据!', function (obj) {
            if (obj == "yes") {
                CS('CZCLZ.Order.DelDriverPay', function (retVal) {
                    if (retVal) {
                        GetCarrierPay(storeCarrierPay.currentPage);
                    }
