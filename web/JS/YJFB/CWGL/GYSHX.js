...
function getUser(nPage) {
    CS('CZCLZ.GYSHXByMySql.GYSHXList', function (retVal) {
        orderstore.setData({
            data: retVal.dt,
            pageSize: pageSize,
            total: retVal.ac,
                                            ...
                                            },
                                            {
                                            ...
                                            },
                                              {
                                                ...
                                                },
                                                {
                                                    ...
                                                    text: '剩余可核销金额',
                                                    renderer: function (value, cellmeta, record, rowIndex, columnIndex, store) {
                                                        return record.data.money - record.data.hxmoney;
                                                    }
                                                }, {
                                                    xtype: 'gridcolumn',
                                                    dataIndex: 'shippingnoteid',
                                                ...
                                                },
                                                 {
                                                    ...
                                                    text: '核销记录',
                                                    renderer: function (value, cellmeta, record, rowIndex, columnIndex, store) {
                                                        return "<div style='color:green;cursor:pointer;' onclick='CKHXJL(\"" + value + "\",\"" + record.data.username + "\",\"" + record.data.actualwaymoney + "\",\"" + record.data.userid + "\")'>查看</div>"; 
                                                    }
                                                }, {
                                                    xtype: 'gridcolumn',
                                                    dataIndex: 'shippingnoteid',
                                                    ...
                                                    text: '操作',
                                                    renderer: function (value, cellmeta, record, rowIndex, columnIndex, store) {
                                                        return "<div style='color:green;cursor:pointer;' onclick='AddPJ(\"" + value + "\",\"" + record.data.username + "\",\"" + record.data.actualwaymoney + "\",\"" + record.data.userid + "\",\"" + record.data.verifymoney + "\",\"" + record.data.offerid + "\",\"" + record.data.costid + "\")'>单独核销</div>";
                                                    }
                                                }
                                                    ...
                                                    },
                                                    {
                                                        ...
                                                        handler: function () {
                                                            ...
                                                            for (var n = 0, len = rds.length; n < len; n++) {
                                                                var rd = rds[n];
                                                                gys += rd.get("username") + ",";
                                                                idlist.push(rd.get("shippingnoteid") + "," + rd.get("userid") + "," + rd.get("offerid") + "," + rd.get("actualwaymoney") + "," + rd.get("verifymoney") + "," + rd.get("username") + "," + rd.get("costid"));
                                                               
                                                                actualwaymoney += (rd.get("actualwaymoney") - rd.get("verifymoney"))
                                                            }
                                                             
                                                            AddPJ2(idlist, gys, actualwaymoney);
                                                        }
                                                    }
                                                ]
                                            },
...
function AddPJ2(idlist, gys, actualwaymoney) {

    if (actualwaymoney != null && actualwaymoney != "" && actualwaymoney != "null") {
               ...
               items: [
                    {
                    ...
                    },
                     {
                         ...
                         decimalPrecision: 5,
                         minValue: 0,
                         maxValue: actualwaymoney,
                         readOnly: true,
                         labelWidth: 90,
                         allowBlank: false,
                         value:actualwaymoney,
                         anchor: '100%'
                     },
               ...
               buttons: [
                   {
                       ...
                       handler: function () {
                           ...
                           Ext.MessageBox.confirm("提示", "总共核销金额" + actualwaymoney + "元，是否核销？", function (obj) {
                               if (obj == "yes") {
                                   var form = Ext.getCmp('addform44');
                                   if (form.form.isValid()) {
                                       var values = form.form.getValues(false);
                                       var me = this;
                                       CS('CZCLZ.GYSHXByMySql.HXMoneyALL', function (retVal) {
                                           if (retVal) {
                                               getUser(1);
...
function AddPJ(notepid, username, actualwaymoney, userid, verifymoney, offerid) {
               ...
               items: [
                    {
                    ...
                    },
                     {
                         ...
                         decimalPrecision: 5,
                         minValue: 0,
                         maxValue: actualwaymoney - verifymoney,
                         labelWidth: 90,
                         allowBlank: false,
                         anchor: '100%'
                     },
                     {
                         xtype: 'label',
                         text: '核销总金额：' + actualwaymoney + ",已核销总金额：" + verifymoney + ",剩余可核销金额：" + (actualwaymoney - verifymoney),
                           ...
                           Ext.MessageBox.confirm("提示", "是否确认核销？", function (obj) {
                               if (obj == "yes") {
                                   ...
                                   if (form.form.isValid()) {
                                       var values = form.form.getValues(false);
                                       var me = this;
                                       CS('CZCLZ.GYSHXByMySql.HXMoney', function (retVal) {
                                           if (retVal) {
                                               getUser(1);
                                               Ext.getCmp("lmwin44").close();

                                            }
                                       }, CS.onError, values, notepid, userid, username, (actualwaymoney - verifymoney), actualwaymoney, offerid, costid);
...
function ZF(billingid, shippingnoteid) {
    CS('CZCLZ.GYSHXByMySql.ChangeZF', function (retVal) {
        if (retVal) {
            getUser(1);
            me.up('window').close();
...
function LookP(userid, actualmoney) {
    ...
    CS('CZCLZ.GYSHXByMySql.GetUserMX', function (retVal) {
        if (retVal.length > 0) {
            var item = retVal[0];
...
function LookDD(shippingnotenumber, actualmoney) {
    ...
    CS('CZCLZ.GYSHXByMySql.GetDDMX', function (retVal) {
        if (retVal.length > 0) {
            var item = retVal[0];
...
function getStartOREnd(shippingnoteid) {
    ...
    CS('CZCLZ.GYSHXByMySql.GetHXLIST', function (retVals) {
            if (retVals.length > 0) {
