...
public class Module
{
    ...
    [CSMethod("SavePrivilegeByPC")]
    public object SavePrivilegeByPC(JSReader jsr)
    {
        using (DBConnection dbc = new DBConnection())
        {
            string privilegeName = jsr["privilegeName"];
            ...
            if (string.IsNullOrEmpty(jsr["privilegeId"].ToString()))
            {
                ...
                var dt = dbc.GetEmptyDataTable("tb_b_privilege_pc");
                var sr = dt.NewRow();
                sr["privilegeId"] = new Guid(privilegeId);
                sr["menuId"] = jsr["menuId"].ToString();
                sr["moduleId"] = jsr["moduleId"].ToString();
    ...
    [CSMethod("DeleteModuleByPC")]
    public bool DeleteModuleByPC(string moduleId)
    {
        using (var db = new DBConnection())
        {
            string sql = "select count(*) num from tb_b_menu_pc where moduleId = @moduleId";
            SqlCommand cmd = db.CreateCommand(sql);
            cmd.Parameters.Add("@moduleId", moduleId);
            string num = db.ExecuteScalar(cmd).ToString();
            if (Convert.ToInt32(num) == 0)
            {
                sql = "delete from tb_b_module_pc where moduleId = @moduleId";
                cmd = db.CreateCommand(sql);
                cmd.Parameters.Add("@moduleId", moduleId);
                db.ExecuteNonQuery(cmd);
            }
    ...
    [CSMethod("DeletePrivilegeByPC")]
    public bool DeletePrivilegeByPC(string privilegeId)
    {
        using (var db = new DBConnection())
        {
            string sql = "delete from tb_b_privilege_pc where privilegeId = @privilegeId";
            SqlCommand cmd = db.CreateCommand(sql);
            cmd.Parameters.Add("@privilegeId", privilegeId);
            db.ExecuteNonQuery(cmd);
            return true;
    ...
    [CSMethod("GetModuleTreeByPC")]
    public object GetModuleTreeByPC()
    {
        using (var db = new DBConnection())
        {
            try
            {
                string sql = "select * from tb_b_module_pc order by modulePx";
                SqlCommand cmd = db.CreateCommand(sql);
                DataTable dt_module = db.ExecuteDataTable(cmd);

                sql = "select * from tb_b_menu_pc order by menuPx";
                cmd = db.CreateCommand(sql);
                DataTable dt_menu = db.ExecuteDataTable(cmd);

                sql = "select * from tb_b_privilege_pc order by privilegePx";
                cmd = db.CreateCommand(sql);
                DataTable dt_privilege = db.ExecuteDataTable(cmd);
    ...
    [CSMethod("DeleteMenuByPC")]
    public bool DeleteMenuByPC(string menuId)
    {
        using (var db = new DBConnection())
        {
            string sql = "select count(*) num from tb_b_privilege_pc where menuId = @menuId";
            SqlCommand cmd = db.CreateCommand(sql);
            cmd.Parameters.Add("@menuId", menuId);
            string num = db.ExecuteScalar(cmd).ToString();
            if (Convert.ToInt32(num) == 0)
            {
                sql = "delete from tb_b_menu_pc where menuId = @menuId";
                cmd = db.CreateCommand(sql);
                cmd.Parameters.Add("@menuId", menuId);
                db.ExecuteNonQuery(cmd);
            }
