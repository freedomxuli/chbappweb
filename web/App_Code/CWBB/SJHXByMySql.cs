    ...
    public object GYSHXList(int pagnum, int pagesize, string stime, string etime, string username,string ddbm,string ishx)
    {
        ...
        using (MySqlDbConnection dbc = MySqlConnstr.GetDBConnection())
        {
            try
            {
                ...
                string str = @"
select * from (
SELECT g.id as costid, h.username AS um,g.money,g.verifymoney AS hxmoney,e.descriptionofgoods,e.itemgrossweight,e.goodsfromroute,e.goodstoroute,e.goodsreceiptplace,
