...
public class SJSHMag
{
  ...
  where a.IsSHPass=1 and a.ClientKind=1 "+ where +@" order by a.AddTime desc,a.UserName,a.UserXM";
    ...
    [CSMethod("GetSJList2")]
    public object GetSJList2(int pagnum, int pagesize, string yhm, string xm, string ispass)
    {
        using (DBConnection dbc = new DBConnection())
        {
            try
            {
                ...
                {
                    tradeCode = "tbbdriverapply.selectApply",
                    status = ispass,
                    userid="",
                    username=yhm,
                    userxm=xm,
    ...
    [CSMethod("GetSJList2ToFile", 2)]
    public byte[] GetSJList2ToFile(string yhm, string xm, string ispass)
    {
        using (DBConnection dbc = new DBConnection())
        {
            try
            {
                ...
                cells[0, 2].SetStyle(style2);
                cells.SetColumnWidth(2, 20);
                cells[0, 3].PutValue("车牌号");
                cells[0, 3].SetStyle(style2);
                cells.SetColumnWidth(3, 20);
                cells[0, 4].PutValue("挂靠单位");           
                cells[0, 4].SetStyle(style2);
                cells.SetColumnWidth(4, 20);
                cells[0, 5].PutValue("后视镜设备编号");
                cells[0, 5].SetStyle(style2);
                cells.SetColumnWidth(5, 20);
                cells[0, 6].PutValue("备注");
                cells[0, 6].SetStyle(style2);
                cells.SetColumnWidth(6, 20);
                ...
                if (list.Length > 0)
                {
                    for (int i = 0; i < list.Length; i++)
                    {
                        ...
                        cells[i + 1, 1].SetStyle(style4);
                        cells[i + 1, 2].PutValue(list[i].UserTel);
                        cells[i + 1, 2].SetStyle(style4);
                        if (list[i].param.carnumber != null && list[i].param.carnumber != "")
                        {
                            cells[i + 1, 3].PutValue(list[i].param.carnumber);
                        }
                        cells[i + 1, 3].SetStyle(style4);

                        if (list[i].param.linkedunit != null && list[i].param.linkedunit != "")
                        {
                            cells[i + 1, 4].PutValue(list[i].param.linkedunit);
                        }
                        cells[i + 1, 4].SetStyle(style4);

                        if (list[i].mirrornumber != null && list[i].mirrornumber != "")
                        {
                            cells[i + 1, 5].PutValue(list[i].mirrornumber);
                        }
                        cells[i + 1, 5].SetStyle(style4);

                        if (list[i].param.drivermemo != null && list[i].param.drivermemo != "")
                        {
                            cells[i + 1, 6].PutValue(list[i].param.drivermemo);
                        }
                        cells[i + 1, 6].SetStyle(style4);
    ...
    [CSMethod("SHSJSB")]
    public object SHSJSB(string userId, string yj)
    {
        string _url = ServiceURL + "tbbdriverapply.reject";
        string jsonParam = new JavaScriptSerializer().Serialize(new
        {
            tradeCode = "tbbdriverapply.reject",
	        id=userId,
            reviewreason = yj
        });
        var request = (HttpWebRequest)WebRequest.Create(_url);
